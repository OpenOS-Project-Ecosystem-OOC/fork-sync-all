...
" 2>/dev/null || true)
  ...
  # Check idempotency
  if [[ "$FORCE" != "true" ]]; then
    already_converted=$(echo "$tree_paths" | grep -E "(^|/)incus-(image|compose)\.yaml$" || true)
    if [[ -n "$already_converted" ]]; then
      info "  Already has Incus files — skipping (use FORCE=true to regenerate)."
      (( skipped++ )) || true
      continue
    fi
  fi
  ...
  while IFS= read -r dockerfile_path; do
    ...
    delete_file "$repo" "$dockerfile_path" \
      "chore(incus): remove Dockerfile (replaced by incus-image.yaml)" \
      "$dockerfile_sha" \
